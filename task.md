...
- [x] Task 6: Wire the login screen to the backend.
...
- [x] Task 7: Fix media delivery on the VPS.
...
- [x] Task 8: Make `ops/deploy-vps.ps1` safe to re-run.
...
- [x] Task 9: Add login rate limiting and proxy awareness.
...
- [x] Task 10: Fix export/import so it cannot lose data.
...
- [x] Task 11: Fail fast on missing `SESSION_SECRET` in production (`server/src/auth.js:35` currently falls back to `""`).
- [x] Task 12: Add graceful shutdown (SIGTERM/SIGINT → `app.close()` + pool close) and a `pool.on("error")` handler so a Postgres restart cannot crash the app (`server/src/server.js:52`, `server/src/db.js:12`). Add `connectionTimeoutMillis` and a statement timeout to the pool.
...
- [x] Task 18: Raise PBKDF2 to 600,000 iterations (OWASP for SHA-256); existing hashes still verify via the stored iteration count. Run `verifyPassword` against a dummy hash when the email is unknown (timing oracle).
- [x] Task 19: Health endpoint returns 503 when the DB ping fails (`server/src/routes/health.js:12-16`); add an app-container `healthcheck` to `compose.yaml`.
- [x] Task 20: Purge expired sessions opportunistically on login (`sessions` table currently grows forever).
...
- [x] Task 22: Add security headers in the Caddyfile (`X-Content-Type-Options`, `X-Frame-Options`) and Docker log rotation (`max-size: 10m`, `max-file: 3`) to all compose services.
